...
export default function CreateTicket ({ auth, errors, functions, clients, seats: allSeats }) {
  ...
  const [inputs, setInputs] = useState([
    ...
    },
    {
      ...
      options: [...functions].map(f => {
        return {
          value: f.id,
          label: f.movie.title + ' - ' + f.hall.name + ' - ' + moment(f.time).format('LLL')
        }
      }
      )
    },
  ...
  return (
    ...
    >
      ...
      <div className='max-w-7xl mx-auto sm:px-6 lg:px-8 flex flex-col flex-wrap gap-5 py-5'>
        ...
        <Form
          routeName='tickets.store' inputs={inputs} initialValues={initialValues} extraOnChanges={
          [{
            input: 'function',
        ...
        />
        <LinkButton href='/tickets' className='w-24 bg-blue-600 dark:bg-blue-300'>go back</LinkButton>
